...
function buildContactUpdatePayload(input: z.infer<typeof patchRequestSchema>) {
  ...
  const linkUpdate: Prisma.ContactCoInvestorUpdateInput = {};
...
export async function POST(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      return { coInvestor, resolved, link };
    });

    return NextResponse.json({
      linked: true,
      coInvestor: result.coInvestor,
...
export async function PATCH(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const link = await tx.contactCoInvestor.findUnique({
        where: { id: input.linkId },
        include: { contact: true }
      });

      if (!link || link.coInvestorId !== coInvestorId) {
      ...
      return tx.contactCoInvestor.findUnique({
        where: { id: link.id },
        include: { contact: true }
      });
    });
    ...
    console.error("co_investor_update_contact_error", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to update co-investor contact" },
      { status: 400 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const removed = await prisma.contactCoInvestor.deleteMany({
      where: {
        id: input.linkId,
        coInvestorId
      }
    ...
    console.error("co_investor_delete_contact_error", error);
    return NextResponse.json({ error: "Failed to delete co-investor contact" }, { status: 400 });
  }
}
